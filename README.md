...
- 多皮肤系统：橘猫/蓝猫/兔子/熊猫/幽灵/小美女，设置里实时预览一键切换
- 角色设定：皮肤决定长相，自我认知单独配置（小美女皮肤 → 小女生），可联动可自定义
...
- 拖动：移动小猫位置
- 右键：立即巡视 / 让它说话 / 设置 / 退出
- 窗口右下角小字：当前状态（待机中 / 巡视中 / 刚看了什么）
- **macOS 状态栏常驻**：app 不在程序坞显示图标，常驻顶部菜单栏（小猫图标）——左键点击显示/隐藏桌宠，右键菜单：显示/隐藏、立即巡视、设置、搜索、退出。macOS 27 上 Qt 托盘点击必崩（QTBUG-147449，Qt 官方未修复），故状态栏用 PyObjC 原生 NSStatusItem 实现（不依赖 Qt 托盘）
...
定时器（默认 10 分钟）
  ...
  → Agent 结合记忆、时间、心情做判断：有值得说的才说话，没有就保持安静（还会私下记想法）
  → 弹出气泡 + 说话动画 + 状态更新
```
...
- **想法日记**：每次巡视即使不说话，也可能把当天看到的东西记成一条想法（`[THINK]` / 规则模式随机记）
- **主动关心**：你之前说过要考试/开会/面试/加班，过一段时间它会主动问进展
- **作息节律**：早上/中午/下午/晚上各问候一次；深夜（默认 23:00–7:00）不主动说话；心情会随天气和时间变化（晴→开心、下雨→有点蔫、深夜→困了）
- **发言冷却**：两次主动说话之间默认间隔 30 分钟，不会十分钟就烦你一次；极端天气提醒不受冷却限制
...
- **行为统计**：聊天消息数、主动发言次数、想法条数、记住的事实条数、巡视次数、在线时长
- **近 7 天走势**：每天模型调用、总 token、对话条数、巡视次数、信息条数
...
- 小橘猫 / 小蓝猫 / 小兔 / 小熊猫 / 小幽灵 / 小美女（棕发、发饰、粉裙）
- 每款皮肤 6 组动作：待机眨眼、说话张嘴、开心跳跃、思考、睡觉（带 Zzz 飘动）、挥手
- 深夜自动睡觉，聊天时思考，主动说话时开心，右键让它说话时会挥手
...
- 设置 → 基本设置里有“自我认知（角色）”字段，例如“小女生”“小橘猫”“我的小管家”，LLM 和规则聊天都会按这个角色说话
- 每款皮肤自带默认角色（小美女 → 小女生、小兔 → 小兔……）
