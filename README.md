...
- **主动行动**：主动思考时可以调用工具（搜索 + 私有工作区），默认最多 10 轮，可在“设置 → 基本 → 主动思考工具轮数”调整；工作区里的写文件和 bash 命令不弹确认，危险命令仍会被拒绝。

### 工作区：Agent 自己的默认文件夹

桌宠有一个完全属于自己的工作区（`<用户数据目录>/workspace`），可以在里面**持续做事**，而不是每次只查一下回一句话：

- `data/observations.db`：每次巡视的插件采集（行情/新闻/天气/名言…）**自动落库**，24 小时内相同条目去重，时间久了就是一张真实的历史数据表
- `projects/`：它生成的产物——股票仪表盘 `dashboard.html`、数据报告、小脚本……
- `notes/`：它的笔记与想法
- 主动思考时，它可以用 `sandbox` 工具：`list/read/write` 操作文件、`run` 执行 shell 命令（仅限工作区内，无需确认）、`db` 对观察库执行 SQL（可以建自己的表做分析）
- 每次思考都会看到工作区现状快照（观察库统计、最近产物），保证跨 tick 连续推进，而不是每次从零开始
- 它干完活可以只输出 `WORK` 一行总结安静记下，也可以顺带 `SPEAK` 一句告诉你（例如“你的股票仪表盘更新啦，在 projects/dashboard.html”）
- 聊天窗左下角“🏠 打开工作区”一键打开这个文件夹；设置里可以整体关闭（“启用桌宠自己的工作区”）
- 单次思考的工具调用次数有预算（默认 12，可调），防止一次巡视刷太久

CLI 支持：`python cli.py workspace status|sync|db|open`。
