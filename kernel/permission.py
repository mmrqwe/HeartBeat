"""kernel.permission：安全边界（工具权限判定与 shell 执行）。

Agent 会执行 bash("rm xxx")，Kernel 必须守门。本模块只做：
命令分级（off/readonly/confirm/full）、敏感路径/环境过滤、硬边界执行，
不依赖任何业务模块（搜索 handler 等技能注册表在 brain 层 tools.py）。
